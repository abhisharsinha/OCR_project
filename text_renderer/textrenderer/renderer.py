...
class Renderer(object):
    ...
    def gen_rand_bg(self, width, height):
        ...
        bg_low = bg_high - random.uniform(1, 100)
    ...
    def reverse_img(self, word_img):
        # If image is coloured change bg color variably
        if len(word_img.shape) == 3:
            temp = word_img
            for i in range(word_img.shape[-1]):
                temp[:, i] = 255 + np.random.randint(50, 100) - temp[:,i]
                temp[:, i] = np.clip(0, 255, temp[:,i])
            return temp
        else:
